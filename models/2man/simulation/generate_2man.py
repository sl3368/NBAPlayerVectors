...
def create_2man_sim_data(N,T,K,filename,players_filename):
    ...
    for i in range(N):
        p1 = random.random_integers(1,T)
        p2 = random.random_integers(1,T)
        while(p1==p2):
        	p2 = random.random_integers(1,T)
        min_played = random.rand(1)
        pts = (alpha+numpy.dot(player_mat[p1-1],numpy.transpose(player_mat[p2-1]))) + (sigma/min_played)*random.randn()
