...
print min(max_mins_norm),max(max_mins_norm)
...
save_data = [player1,player2,points_val,max_mins_norm,players,index_to_player_dict]
sf = open(savefilename,'wb')
cPickle.dump(save_data,sf, protocol=cPickle.HIGHEST_PROTOCOL)
sf.close()

if rdump:
    data_dict = { "N":len(player1), "T":len(players), "K":k_size, "player1":player1, "player2":player2, "y":points_val, "mins":max_mins_norm }
