...
for k in range(FOLDS):
    ...
    for i in ts_idx:
        ts_player1.append(player1[i])
        ts_player2.append(player2[i])
        ts_mins.append(norm_mins[i])
        ts_pts.append(pts[i])

    
    # SAVE TRAINING DATA IN THE CORRECT FORMAT
    data_dict = { "N":len(tr_player1), "T":len(all_players), "K":k_val, "player1":tr_player1, "player2":tr_player2, "y":tr_pts, "mins":tr_mins }
    pystan.misc.stan_rdump(data_dict,directory+"CV/"+str(k)+"_fold_"+str(k_val)+".data.R")

    # SAVE THE TESTING DATA
    testing_data = [ts_player1,ts_player2,ts_mins,ts_pts]
    sf = open(directory+"CV/"+str(k)+"_fold_test_"+str(k_val)+".data",'wb')
