...
### FOR 5 MAN LINEUPS
### Command line arguments [object_path] [output_path] [rdump_path] [k vector size]
        ...
        if len(names)==5:
            if len(lineup[19])>0:
                if len(lineup[5])>0:
                    ...
                    player4.append(two_men[3]+1)
                    player5.append(two_men[4]+1)
...
save_data = [player1,player2,player3,player4,player5,points_val,max_mins_norm,players,index_to_player_dict]
sf = open(savefilename,'wb')
cPickle.dump(save_data,sf, protocol=cPickle.HIGHEST_PROTOCOL)
sf.close()

if rdump:
    data_dict = { "N":len(player1), "T":len(players), "K":k_size, "player1":player1, "player2":player2, "player3":player3, "player4":player4, "player5":player5, "y":points_val, "mins":max_mins_norm }
